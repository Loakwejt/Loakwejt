...
export default function WorkspaceDebitNotesPage() {
  ...
  const [editNote, setEditNote] = useState<DebitNote | null>(null);
  const [lineItems, setLineItems] = useState<LineItem[]>([{ description: '', quantity: 1, unitPrice: 0, total: 0 }]);
  const [taxRate, setTaxRate] = useState(19);

  const baseUrl = `/api/workspaces/${params.workspaceId}/debit-notes`;
  ...
  const loadDebitNotes = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) { console.error('Debit notes API error:', res.status); return; }
  ...
  useEffect(() => {
    loadDebitNotes();
  }, [search, statusFilter, loadDebitNotes]);
  ...
  async function handleDelete(id: string) {
    if (!baseUrl || !confirm('Belastungsanzeige wirklich löschen?')) return;
  ...
  async function handleVoid(id: string) {
    if (!baseUrl || !confirm('Belastungsanzeige wirklich stornieren?')) return;
    await fetch(`${baseUrl}/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function handleSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      debitNoteNumber: form.get('debitNoteNumber') as string,
  ...
  function openEdit(note: DebitNote) {
    setEditNote(note);
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Belastungsanzeigen</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Verwalte Belastungsanzeigen für Nachbelastungen und Korrekturen. {total > 0 && `(${total} gesamt)`}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Button onClick={() => { setEditNote(null); setLineItems([{ description: '', quantity: 1, unitPrice: 0, total: 0 }]); setShowForm(true); }} className="gap-2">
            <Plus className="h-4 w-4" /> Neue Belastungsanzeige
          </Button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative max-w-sm flex-1">
          ...
          <Input
            placeholder="Belastungsnummer oder Bestellung suchen…"
            value={search}
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle>{editNote ? 'Belastungsanzeige bearbeiten' : 'Neue Belastungsanzeige'}</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="debitNoteNumber">Belastungsnummer *</Label>
                  <Input id="debitNoteNumber" name="debitNoteNumber" placeholder="BA-2026-001"
                    defaultValue={editNote?.debitNoteNumber || ''} required />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2 sm:col-span-2">
                  ...
                  <Textarea id="reason" name="reason" placeholder="Grund für die Belastungsanzeige…"
      ...
      {/* List */}
      {loading ? (
        <div className="text-center py-12 text-muted-foreground">Lade Belastungsanzeigen…</div>
      ) : debitNotes.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            <FilePlus className="h-12 w-12 text-muted-foreground mb-4" />
            <p className="text-lg font-medium">Noch keine Belastungsanzeigen</p>
            <p className="text-sm text-muted-foreground mt-1">Erstelle deine erste Belastungsanzeige.</p>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={note.id}>
              <CardContent className="flex items-center justify-between py-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-bold">{note.debitNoteNumber}</span>
