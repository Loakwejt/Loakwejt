...
// POST /api/runtime/workspaces/[slug]/auth/forgot-password
...
export async function POST(
  ...
  try {
    ...
    await prisma.siteUserPasswordReset.updateMany({
      where: { workspaceId: workspace.id, email: email.toLowerCase(), used: false },
      data: { used: true },
    });
    ...
    const resetUrl = `${baseUrl}/s/${slug}/reset-password?token=${token}`;
    ...
    await createAuditLog({
      ...
      details: { slug, email: email.toLowerCase() },
    });
