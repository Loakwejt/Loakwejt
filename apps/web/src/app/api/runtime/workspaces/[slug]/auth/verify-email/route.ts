...
// GET /api/runtime/workspaces/[slug]/auth/verify-email?token=...
...
export async function GET(
  ...
  try {
    ...
    if (siteUser.emailVerified) {
      return new NextResponse(successPage('Deine E-Mail-Adresse wurde bereits bestätigt.', slug), {
        status: 200,
        headers: { 'Content-Type': 'text/html; charset=utf-8' },
      });
    }
    ...
    await createAuditLog({
      ...
      details: { slug, email: siteUser.email },
    });

    return new NextResponse(successPage('E-Mail erfolgreich bestätigt! Du kannst dich jetzt anmelden.', slug), {
      status: 200,
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
    });
  } catch (error) {
...
function successPage(message: string, slug: string) {
  return `<!DOCTYPE html>
<html lang="de">
<head>
  <meta charset="utf-8" />
